...
/**
 * A note from Matthew Lindeman, this file has been edited to have no memory
 * leaks/errors and split from just a .h file to a .c file as well.
 */
#include "include/objpar.h"
...
unsigned int objpar(const char* p_string, unsigned int string_size, void* p_buffer, struct objpar_data* p_data)
{
    ...
    while (index < string_size)
    {
        ...
        else if (objpar_internal_comment(p_string, &index));
        else objpar_internal_newline(p_string, &index, 0);
    }
    ...
    while (index < string_size)
    {
        ...
        else objpar_internal_newline(p_string, &index, OBJPAR_NULL(unsigned int));
    }
    return 1;
}
...
unsigned int objpar_internal_v(const char* p_string, unsigned int* p_index, unsigned int string_size, float** pp_vbuff, unsigned int vertex_width)
{
    ...
    if (c0 == 'v' && c1 == ' ')
    {
        if (pp_vbuff == OBJPAR_NULL(float*))
        {
            ...
            objpar_internal_newline(p_string, p_index, &space_count);
            return space_count;
        }
